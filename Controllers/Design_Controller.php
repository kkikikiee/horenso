...
class Design_Controller extends Controller
{
    ...
    // 이미지 배열 반환 메서드
    public function change_to_img_array($value_parameter){

        //return $value_parameter;
        return $this->obj->Change_to_array2($value_parameter);
    }

///////////////////////////////////////////////////

    // 도안생성 메서드
    public function Change_to_image_WebMyPento($array_result){

        foreach ($array_result as $key => $value){

            $this->image[$value['design_no']] = imagecreatetruecolor(300,300);
            $background = imagecolorallocate($this->image[$value['design_no']],254,251,239);
            imagefill($this->image[$value['design_no']],0,0,$background);

            for($j=10*2; $j<=300; $j+=20){
                $white = imagecolorallocate($this->image[$value['design_no']],255,255,255);
                imageline($this->image[$value['design_no']],$j,0,$j,300,$white);
                imageline($this->image[$value['design_no']],0,$j,300,$j,$white);
            }
        }
        ...
        foreach ($array_result as $key => $value){
                ...
                for ($x = 0, $count = 0; $x < 15; $x++) {
                    ...
                    for ($y = 0; $y < 15; $y++) {
                        ...
                        if ($y == $value['board_Y'] && $x == $value['board_X']) {

                            $random = rand(0, count($color_ran) - 1);

                            $color_cube = imagecolorallocate(
                                $this->image[$value['design_no']], $color_ran[$random][0], $color_ran[$random][1], $color_ran[$random][2]);

                            imagefilledrectangle($this->image[$value['design_no']],
                                $x * 20, $y * 20,
                                ($x * 20) + 20, ($y * 20) + 20,
                                $color_cube
                            );
...
class Design_Change {
    ...
    public function Change_to_array2($array_db){
        // DB 좌표 -> img 좌표
	
        $array_img_result = array();

        foreach ($array_db as $key1 => $value1) {
            ...
            foreach ($value1 as $key2 => $value2) {
                ...
                switch ($key2) {
                    ...
                    case 'design_no':
                        $array_img_result[$key1][$key2] = $value2;
                        break;
                    case 'board_X':
                        $array_img_result[$key1][$key2] = $value2;
                        break;
...
////////////////////////////////////////////////////////////
	...
	if( count($this->image) != 1 ){	
 	   ...
 	   imagejpeg($this->image[($array_image[$i]['design_no']-1)],$_SERVER['DOCUMENT_ROOT'] . "/images/collection/pentoimg".($array_image[$i]['design_no']).".png");

            $array_image[($array_image[$i]['design_no']-1)]['file_name'] = "http://ec2-13-125-219-201.ap-northeast-2.compute.amazonaws.com" . "/images/collection/pentoimg".($array_image[$i]['design_no']).".png";
	}
	else{

	   imagejpeg($this->image[$i],$_SERVER['DOCUMENT_ROOT'] . "/images/collection/pentoimg".($array_image[$i]['design_no']).".png");

            $array_image[$i]['file_name'] = "http://ec2-13-125-219-201.ap-northeast-2.compute.amazonaws.com" . "/images/collection/pentoimg".($array_image[$i]['design_no']).".png";
	}
